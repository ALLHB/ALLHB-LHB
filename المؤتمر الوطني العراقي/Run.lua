...
ALLHB-LHB = Token:match("(%d+)")
redis:set(ALLHB-LHB..":VERSION",2.9)
redis:set(ALLHB-LHB..":SUDO_ID:",SUDO_USER)
redis:set(ALLHB-LHB..":DataCenter:",'German')
redis:set(ALLHB-LHB..":UserNameBot:",BOT_User)
redis:set(ALLHB-LHB..":NameBot:",BOT_NAME)
redis:hset(ALLHB-LHB..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3ALLHB-LHB_INSTALL","Yes")
info = {}
...
ALLHB-LHB = Token:match("(%d+)")
our_id = tonumber(ALLHB-LHB)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(ALLHB-LHB..":UserNameBot:")
SUDO_ID = tonumber(redis:get(ALLHB-LHB..":SUDO_ID:"))
SUDO_USER = redis:hgetall(ALLHB-LHB..'username:'..SUDO_ID).username
version = redis:get(ALLHB-LHB..":VERSION")
DataCenter = redis:get(ALLHB-LHB..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..ALLHB-LHB..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(ALLHB-LHB..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(ALLHB-LHB..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(ALLHB-LHB..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(ALLHB-LHB..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(ALLHB-LHB..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي👷🏽'
msg.TheRank = 'المنشىء الاساسي👷🏽 '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(ALLHB-LHB..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(ALLHB-LHB..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(ALLHB-LHB..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(ALLHB-LHB..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(ALLHB-LHB..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(ALLHB-LHB..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(ALLHB-LHB..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(ALLHB-LHB..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(ALLHB-LHB..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(ALLHB-LHB..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(ALLHB-LHB..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(ALLHB-LHB..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(ALLHB-LHB..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(ALLHB-LHB..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.ALLHB-LHB then 
if msg.text and ScriptFile.iALLHB-LHB then
for k, ALLHB-LHB in pairs(ScriptFile.ALLHB-LHB) do
local SearchText = msg.text:match(ALLHB-LHB)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iALLHB-LHB(msg,{msg.text:match(ALLHB-LHB)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ALLHB-LHB.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dALLHB-LHB then
if ScriptFile.dALLHB-LHB(msg) == false then
return false
end
...
if Plug.ALLHB-LHB then 
if msg.text and Plug.iALLHB-LHB then
for k, ALLHB-LHB in pairs(Plug.ALLHB-LHB) do
local SearchText = msg.text:match(ALLHB-LHB)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(ALLHB-LHB..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(ALLHB-LHB..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iALLHB-LHB(msg,{msg.text:match(ALLHB-LHB)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ALLHB-LHB.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dALLHB-LHB then
Plug.dALLHB-LHB(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source ALLHB-LHB")
end 

end
else
print("The File Script.lua Not Runing in The Source ALLHB-LHB")
...
function tdcli_update_callback(data)
	...
	redis:srem(ALLHB-LHB..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/ALLHB-LHBTELLE/ALLHB-LHB/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/ALLHB-LHBTELLE/ALLHB-LHB/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/ALLHB-LHBTELLE/ALLHB-LHB/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/ALLHB-LHBTELLE/ALLHB-LHB/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/ALLHB-LHBTELLE/ALLHB-LHB/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/ALLHB-LHBTELLE/ALLHB-LHB/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(ALLHB-LHB..":SUDO_ID:")
redis:hset(ALLHB-LHB..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(ALLHB-LHB..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(ALLHB-LHB..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(ALLHB-LHB..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(ALLHB-LHB..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(ALLHB-LHB..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ALLHB-LHB..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ALLHB-LHB..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(ALLHB-LHB..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(ALLHB-LHB..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(ALLHB-LHB..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(ALLHB-LHB..'group:add'..msg.chat_id_) then
	redis:set(ALLHB-LHB..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(ALLHB-LHB..'group:add'..data.chat_.id_) then
	redis:set(ALLHB-LHB..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(ALLHB-LHB..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(ALLHB-LHB..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(ALLHB-LHB..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	ALLHB-LHB..'group:name'..k,IDS.Title,
	ALLHB-LHB..'num_msg_ALLHB-LHB'..k,5,
	ALLHB-LHB..'group:add'..k,true,
	ALLHB-LHB..'lock_link'..k,true,
	ALLHB-LHB..'lock_spam'..k,true,
	ALLHB-LHB..'lock_webpage'..k,true,
	ALLHB-LHB..'lock_markdown'..k,true,
	ALLHB-LHB..'lock_flood'..k,true,
	ALLHB-LHB..'lock_bots'..k,true,
	ALLHB-LHB..'mute_forward'..k,true,
	ALLHB-LHB..'mute_contact'..k,true,
	ALLHB-LHB..'mute_document'..k,true,
	ALLHB-LHB..'mute_inline'..k,true,
	ALLHB-LHB..'lock_username'..k,true,
	ALLHB-LHB..'replay'..k,true
	)
	redis:sadd(ALLHB-LHB..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(ALLHB-LHB..'username:'..ID,'username',user)
	redis:sadd(ALLHB-LHB..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(ALLHB-LHB..'username:'..ID,'username',user)
	redis:sadd(ALLHB-LHB..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(ALLHB-LHB..'username:'..ID,'username',user)
	redis:sadd(ALLHB-LHB..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(ALLHB-LHB..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(ALLHB-LHB..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(ALLHB-LHB..'username:'..data.user_.id_).username
	...
	redis:hset(ALLHB-LHB..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(ALLHB-LHB..'group:ids')
	...
	local pv = redis:smembers(ALLHB-LHB..'users')
	local NumPvDel = 0
	for i = 1, #pv do
